...
async function getTheTotalNumberOfSubmittedOrders() {
    ...
    try {
        ...
        console.error(`[DB_ERROR] Method: getTheTotalNumberOfSubmittedOrders | Error: ${error.message}`);
...
async function getCityNameFromServer(id) {
    ...
    try {
        ...
        console.error(`[DB_ERROR] Method: getCityNameFromServer | UserID: ${id} | Error: ${error.message}`);
        throw new ServerError(ErrorType.GENERAL_ERROR);
    }
}
...
async function getStreetNameFromServer(id) {
    ...
    try {
        ...
        console.error(`[DB_ERROR] Method: getStreetNameFromServer | UserID: ${id} | Error: ${error.message}`);
        throw new ServerError(ErrorType.GENERAL_ERROR);
    }
}
...
async function sameDateShipeDate() {
    ...
    try {
        ...
        console.error(`[DB_ERROR] Method: sameDateShipeDate | Error: ${error.message}`);
        throw new ServerError(ErrorType.GENERAL_ERROR);
    }
}
...
async function getLastOrder(id) {
    ...
    try {
        ...
        console.error(`[DB_ERROR] Method: getLastOrder | UserID: ${id} | Error: ${error.message}`);
        throw new ServerError(ErrorType.GENERAL_ERROR);
    }
}
...
async function updateUserCartDetails(orderDetails) {
    ...
    try {
        ...
        console.log(`[DB_EVENT] Cart status updated to closed | CartID: ${orderDetails[0].id}`);
        return updateUserCartDetailsResponseFromServer
    } catch (error) {
        console.error(`[DB_ERROR] Method: updateUserCartDetails | CartID: ${orderDetails[0].id} | Error: ${error.message}`);
        throw new ServerError(ErrorType.GENERAL_ERROR);
    }
}
...
async function insertOrderInServer(userDetails, generalOrderDetails, openedCartDetails, itemsOfCartDetails) {
    ...
    try {
        ...
        console.log(`[DB_EVENT] New order inserted | UserID: ${userDetails[0].id} | CartID: ${openedCartDetails[0].id} | TotalPrice: ${cartTotalPrice}`);
        return addOrderResponseFromServer
    } catch (error) {
        console.error(`[DB_ERROR] Method: insertOrderInServer | UserID: ${userDetails[0].id} | Error: ${error.message}`);
        throw new ServerError(ErrorType.GENERAL_ERROR);
    }
}
...
function calculateTotalPrice(itemsOfCart) {
    ...
    // השארתי את הלוג שלך לצורך המשכיות
...
async function getLastOrderDate(userDetails) {
    ...
    try {
        ...
        console.error(`[DB_ERROR] Method: getLastOrderDate | UserID: ${userDetails[0].id} | Error: ${error.message}`);
        throw new ServerError(ErrorType.GENERAL_ERROR);
    }
}
...
async function getReceipt(myCartDetails, userDetails, itemsOfCart) {
    ...
    try {
        ...
        console.log(`[FILE_EVENT] Receipt created | FileName: ${fileName}`);
    } catch (fileError) {
        console.error(`[FILE_ERROR] Failed to create receipt file | Error: ${fileError.message}`);
    }
    ...
    try {
        ...
        console.log(`[DB_EVENT] Receipt path updated in DB | CartID: ${myCartDetails.cart_id}`);
    } catch (error) {
        console.error(`[DB_ERROR] Method: getReceipt (update) | CartID: ${myCartDetails.cart_id} | Error: ${error.message}`);
        throw new ServerError(ErrorType.GENERAL_ERROR);
    }
    return fileObj
}
